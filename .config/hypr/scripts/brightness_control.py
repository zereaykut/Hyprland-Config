...
class BrightnessControl:
    ...
    def send_notification(self):
        brightness_val = int(
            sp.run(
                "brightnessctl get", shell=True, capture_output=True, text=True
            ).stdout.strip()
        )
        brightness_max = int(
            sp.run(
                "brightnessctl max", shell=True, capture_output=True, text=True
            ).stdout.strip()
        )
        ...
        sp.run(
            f"""notify-send "Brightness {brightness_percent}%" -i {icon} -r 91190 -t 800 -u normal""",
            shell=True,
        )
