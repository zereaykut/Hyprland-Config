...
class ColorThief(object):
    ...
    def get_color(img, quality=10):
        ...
        :return tuple: (r, g, b)
        """
        palette = ColorThief.get_palette(img, 5, quality)
        return palette[0]
    ...
    def get_palette(img, color_count=10, quality=10):
        ...
        :param img: A filename (string) or a file object
    ...
    def gen_colors(img):
        """
        Format the output from get_palette into a list of hex colors.
        """
        for i in range(11):
            palette = ColorThief.get_palette(img, color_count=11 + 1)
            ...
            print(f"ColorThief couldn't generate a palette. Trying a larger palette size {10 + i}")

        return palette

    @staticmethod
    def run(img):
        palette = ColorThief.get_palette(img, color_count=11)
        return [ColorSys.rgb_to_hex(color) for color in palette]
