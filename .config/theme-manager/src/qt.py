...
def qt5ct(config, theme, user):
    with open(f"/home/{user}/.config/theme-manager/conf/qt5ct.conf", "r", encoding="UTF-8") as f:
        qt5ct_config = f.read()
...
def qt6ct(config, theme, user):
    with open(f"/home/{user}/.config/theme-manager/conf/qt6ct.conf", "r", encoding="UTF-8") as f:
        qt6ct_config = f.read()
