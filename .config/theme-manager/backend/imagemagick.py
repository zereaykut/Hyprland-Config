...
class ColorSchemeGenerator:
    ...
    def __init__(self, user):
        self.user = user
        self.base_dir = f"/home/{self.user}/.cache/color_schemes"
    ...
    def gen_colors(self, img):
        ...
        """
        for i in range(20):
            raw_colors = self.imagemagick(16 + i, img)
    ...
    def adjust(self, colors, light):
        ...
        if light:
            ...
            raw_colors[0] = self.lighten_color(colors[-1], 0.85)
            raw_colors[7] = colors[0]
            raw_colors[8] = self.darken_color(colors[-1], 0.4)
            raw_colors[15] = colors[0]
        else:
            raw_colors[0] = self.darken_color(raw_colors[0], 0.40)
            raw_colors[7] = self.blend_color(raw_colors[7], "#EEEEEE")
            raw_colors[8] = self.darken_color(raw_colors[7], 0.30)
            raw_colors[15] = self.blend_color(raw_colors[15], "#EEEEEE")
    ...
    def get_color_scheme(self, img, theme, output_name, light=False):
        ...
        """
        colors = self.gen_colors(img)
        adjusted_colors = self.adjust(colors, light)
